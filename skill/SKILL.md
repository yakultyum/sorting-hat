...
- **终端命令**：`/sorting-hat init`，在生成 CLAUDE.md 的同时自动写入 `.sorting-hat/project.md`（两个文件共用同一次问答收集的信息）
- **网页**：档案页 → 展开任意 profile → 「项目记忆」区块 → 新建
- **服务端直接调用**：`POST /projects`，接收 `{ project_dir, profile_id, name, tech_stack, conventions, gotcha, persona_override }`
...
CLAUDE.md 写入成功后，紧接着生成 `.sorting-hat/project.md`。

从已收集的信息中提取：
- **name**：项目目录名或项目用途的第一句话
- **tech_stack**：第二步确认的技术栈
- **conventions**：第四步收集的命名规范 + 注释语言，合并为一段
- **gotcha**：第五步收集的内容
- **persona_override**：留空（用户后续可在档案页补充）

调用服务端写入：

```bash
curl -s -X POST http://localhost:7432/projects \
  -H "Content-Type: application/json" \
  -d '{"project_dir": "<目标路径>", "name": "<项目名>", "tech_stack": "<技术栈>", "conventions": "<约定>", "gotcha": "<gotcha>", "persona_override": ""}'
```

如果服务器未运行（`/save` 端点不可达），改用 Write 工具直接写入 `<目标路径>/.sorting-hat/project.md`，内容格式见路径 G 的「文件结构」。

写入成功后输出：

```
✓  已写入 [目标路径]/.sorting-hat/project.md
   AI 进入这个项目时会自动读取项目记忆，覆盖全局人设对应准则。
   如需补充人设覆盖或修改内容，打开档案页 → 展开 profile → 项目记忆。
```
